...
public class WeChat extends JFrame implements ActionListener,MouseListener {
	...
	private JLabel cd_touxiang,cd_exit,cd_min,cd_yuyin,cd_shipin,cd_transfile,cd_createteam,cd_yuancheng,cd_sharedisplay;
	private JButton cd_searchjilu,cd_jiacu,cd_xiahuaxian,cd_color,cd_xieti;
	private JComboBox<String> cd_ziti,cd_size;
    ...
    private final JLabel label1 = new JLabel(new ImageIcon("image/dialogimage/right-pic.jpg"));
	class PicInfo{
		/* 图片信息*/
		int pos;
		String val;
	...
	public WeChat (String owner,String ownerName,String friend) {
		...
		this.addWindowListener(new WindowAdapter() {
			public void windowClosing(WindowEvent e) {
				ManageWeChat.RemoveWeChat(WeChat.this.ownerId+" "+WeChat.this.friendId);
			}
		});
		
		FriendList mylist =  ManageFriendList.getFriendList(ownerId);
		Enumeration<?> enumeration;
		enumeration = mylist.root.preorderEnumeration();
		while (enumeration.hasMoreElements()) { // 遍历枚举对象
			// 先定义一个节点变量,将节点名称给node
		...
		cd_touxiang.addMouseListener(new MouseAdapter() {
			...
			public void mouseEntered(MouseEvent e) {
				cd_touxiang.setCursor(new Cursor(Cursor.HAND_CURSOR));
			}
		});
		...
		friendname.setBounds(62, 10, 150, 20);
		friendname.setFont(new Font("微软雅黑", Font.PLAIN, 15));
		friendname.addMouseListener(new MouseAdapter() {
			...
			public void mouseEntered(MouseEvent e) {
				...
				friendname.setText("<html><u>"+WeChat.this.Friendname+"</u></html>");
			}
			public void mouseExited(MouseEvent e) {
				friendname.setText(WeChat.this.Friendname);
			}
		});
		...
		cd_transfile.addMouseListener(new MouseAdapter() {
			public void mouseClicked(MouseEvent e) {
				...
				fileDialog.showOpenDialog(WeChat.this);
				File file=fileDialog.getSelectedFile();
				Message m=new Message();
				m.setMesType(MessageType.message_sendfile);
				m.setSender(WeChat.this.ownerId);
				m.setGetter(WeChat.this.friendId);
				m.setMyname(WeChat.this.ownerName);
				...
				try {
				    ...
				    oos.writeObject(m);
					info="您已成功向 "+WeChat.this.Friendname+" 发送文件\""+file.getName()+"\""+"\r\n";
			    } catch (Exception e1) {
			    	info="服务器连接超时，文件发送失败\r\n";
			    }
				if(!info.equals(null)&&!info.equals(""))
					WeChat.this.showfileinfoMsg(m, info);
			}
		});
		...
		cd_close.addActionListener(this);
	...
	public void showownMessage(Message m){
		String info=m.getMyname()+"  "+m.getSendTime()+"\n";
		SimpleAttributeSet attrset = new SimpleAttributeSet();
        StyleConstants.setFontSize(attrset,14);
        StyleConstants.setForeground(attrset, Color.green);
        StyleConstants.setFontFamily(attrset, "宋体");
	...
	public void shake(Message m){
		...
		new Thread(){
			...
			Point p = WeChat.this.getLocationOnScreen();
			public void run(){
				int i = 1;
				while((end-begin)/1000<2){
					WeChat.this.setLocation(new Point((int)p.getX()-5*i,(int)p.getY()+5*i));
					end = System.currentTimeMillis();
					try {
						Thread.sleep(5);
						i=-i;
						WeChat.this.setLocation(p);
	...
	public void actionPerformed(ActionEvent e) {
		...
		if(e.getSource()==cd_zhendong) {
			...
			m.setSender(this.ownerId);
			m.setGetter(this.friendId);
			m.setMyname(this.ownerName);
			m.setFriname(this.Friendname);
			m.setCol(Color.red.toString());
			m.setFont("黑体");
			m.setSize(14);
			DateFormat ddtf = DateFormat.getDateTimeInstance();  
			m.setSendTime(ddtf.format(new Date()));
			m.setCon("\r\n"+this.ownerName+" 给你发送了一个窗口抖动！\r\n");
			String info="\r\n你向 "+this.Friendname+" 发送了一个窗口抖动！\r\n";
			SimpleAttributeSet attrset = new SimpleAttributeSet();
	        StyleConstants.setFontSize(attrset,14);
	        StyleConstants.setForeground(attrset, Color.green);
	        StyleConstants.setFontFamily(attrset, "宋体");
